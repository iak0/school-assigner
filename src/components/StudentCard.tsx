...
export const StudentCard: React.FC<StudentCardProps> = ({
  ...
  const topChoices = (preferences ?? []).slice(0, 3).map((rId, idx) => {
    ...
    return <CompactPreferencePill key={rId} role={r} rankIndex={idx} />;
  });
    ...
    >
      <div className="flex items-center justify-between gap-1.5 pointer-events-none">
        <div className="flex items-center gap-1.5 min-w-0 flex-1">
          ...
          <span className="font-bold text-slate-900 text-xs truncate">{studentName}</span>
          {showDragOver && (
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          ...
          {showRepeatWarning && repeatWarningDate && (
            <span
              className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[10px] font-medium bg-rose-100 text-rose-800 border border-rose-300 flex-shrink-0"
              ...
              {isLocked ? (
                ...
                <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path
                    ...
                    strokeLinejoin="round"
                    strokeWidth={2.5}
                    d="M15 7h3a2 2 0 012 2v10a2 2 0 01-2 2h-3M9 7H6a2 2 0 01-2-2V7a2 2 0 012-2h3m6 0H9m4 0H9m-2 4v8a2 2 0 01-2 2H9a2 2 0 01-2-2v-8"
                  />
                </svg>
              )}
            </button>
      ...
      {/* Top 3 Choice Badges */}
      <div className="flex flex-wrap gap-1 mt-1 pl-4.5 pointer-events-none">{topChoices}</div>
